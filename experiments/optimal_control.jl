# This script formulates and solves an optimal control problem (OCP) for a glucose-insulin dynamics model.
# The OCP uses posterior samples obtained from a staged Metropolis-Hastings sampler with ODE-integrated latent trajectories.
# The performance of the proposed method is compared to several baseline methods.
...
# Plot EKF estimates.
#=
p_ekf_1 = plot(t_m, x[1, :], label="true", lw=2)
plot!(t_m, x_hat_hist[1, :], label="EKF estimate", lw=2)
xlabel!("t")
ylabel!("x_1")
display(p_ekf_1)
p_ekf_2 = plot(t_m, x[2, :], label="true", lw=2)
plot!(t_m, x_hat_hist[2, :], label="EKF estimate", lw=2)
xlabel!("t")
ylabel!("x_2")
display(p_ekf_2)
p_ekf_3 = plot(t_m, x[3, :], label="true", lw=2)
plot!(t_m, x_hat_hist[3, :], label="EKF estimate", lw=2)
xlabel!("t")
ylabel!("x_3")
display(p_ekf_3)
=#
...
# @printf("Prior OCP (input constraints):     %s\n", h_u_satisfied_prior ? "Yes" : "No")
@printf("No control (state constraints):    %s\n", h_scenario_satisfied_no_control ? "Yes" : "No")
@printf("No control (input constraints):    %s\n", h_u_satisfied_no_control ? "Yes" : "No")
@printf("Bolus input (state constraints):   %s\n", h_scenario_satisfied_bolus ? "Yes" : "No")
@printf("Bolus input (input constraints):   %s\n", h_u_satisfied_bolus ? "Yes" : "No")
println("====================================================\n")

# Plot the predictions and the true trajectory.
# Compare against comparison methods.
states_to_plot = [1]
...
data2txt("optimal_control.txt", "t", t_pred[idx],
    "x1_min_constraint", x_min_constraint[1, idx],
    "x1_max_constraint", x_max_constraint[1, idx],
    "x1_min", x1_min[idx],
    "x1_max", x1_max[idx],
    "x1_mean", x1_mean[idx],
